...
void LAppWavFileHandler::Start(unsigned char* bytes, size_t len)
{
    // WAVファイルのロード
    if (!LoadWavData(bytes, len))
    {
        return;
    }
...
Csm::csmBool LAppWavFileHandler::LoadWavData(unsigned char* bytes, size_t len)
{
    Csm::csmBool ret;
    ...
    // ファイルロード
    _byteReader._fileByte = bytes;
    _byteReader._fileSize = len;
